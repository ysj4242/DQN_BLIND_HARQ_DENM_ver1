...
for j=1:phyParams.cv2xNumberOfReplicasMax    
    subframesToNextAlloc(:,j) = (subframeNextResource(:,j)>currentT).*(subframeNextResource(:,j)-currentT)+(subframeNextResource(:,j)<=currentT).*...
    (subframeNextResource(:,j)+appParams.NbeaconsT-currentT);
