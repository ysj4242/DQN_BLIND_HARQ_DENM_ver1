...
%         sinr = sinr / 1000;
        ...
        expManagement.reward(scheduledID(indexSensingV),1) = stationManagement.PDR(scheduledID(indexSensingV)) - (0.1 * stationManagement.resource_col(scheduledID(indexSensingV)));
...
%     end
    if simParams.resourceReEvaluation == true
        if sum((scheduledID_ReEval==scheduledID(indexSensingV))==1)
            ...
            if BRtoChange(1,1)~=0
%                 BR=BRidOriginal(1,1);   % maintains first resource
                stationManagement.resource_col(scheduledID(indexSensingV),1) = 1;
                continue
            end
            if stationManagement.cv2xNumberOfReplicas(scheduledID(indexSensingV)) >= 2
                if BRtoChange(1,2)~=0
                    %                 BR=BRidOriginal(1,2);   % maintains second resource
                    stationManagement.resource_col(scheduledID(indexSensingV),2) = 1;
                    continue
                end
            end
            if stationManagement.cv2xNumberOfReplicas(scheduledID(indexSensingV)) >= 3
                if BRtoChange(1,3)~=0
                    ...
                    stationManagement.resource_col(scheduledID(indexSensingV),3) = 1;
                    continue
                end
            end
            1이면 첫번째 자원 충돌 2이면 2번째 자원 충돌 3이면 3번째 자원 충돌
        end
    end
