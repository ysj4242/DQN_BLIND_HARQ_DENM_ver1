...
if ~isempty(stationManagement.transmittingIDsCV2X)
    for i = 1:length(stationManagement.indexInActiveIDsOnlyLTE_OfTxLTE)
        ...
        hasFirstResourceThisTbeacon = ceil(stationManagement.BRid(idVtx,1)/appParams.NbeaconsF)==currentT; % 첫 전송
                              
        if stationManagement.cv2xNumberOfReplicas(idVtx)>1 && hasFirstResourceThisTbeacon && simParams.mode5G
            % BR of the next reserved resource (blind retransmission)
            BRnextReservedTransmission=stationManagement.BRid(idVtx,2); %두번 째 전송
        else
            BRnextReservedTransmission=-1;
        end

        if stationManagement.cv2xNumberOfReplicas(idVtx)>2 && hasFirstResourceThisTbeacon && simParams.mode5G
            % BR of the next reserved resource (blind retransmission)
            BRnextnextReservedTransmission=stationManagement.BRid(idVtx,3); %두번 째 전송
        else
            BRnexnexttReservedTransmission=-1;
        end
            
        for indexNeighborsOfVtx = 1:length(stationManagement.neighborsIDLTE(indexVtxLte,:)) % 송신 차량의 수신 차량 번호
           ...
           if stationManagement.correctSCImatrixCV2X(i,indexNeighborsOfVtx) == 1                
               ...
               if stationManagement.resReselectionCounterCV2X(idVtx)>1 % RC가 1보다 크면 KnownUsedMatrix에 1로 저장 아니면 0으로 저장
...
%                % if HARQ is active in 5G -> reserve the next retransmission
               if stationManagement.cv2xNumberOfReplicas(idVtx)>1 && hasFirstResourceThisTbeacon && BRnextReservedTransmission~=-1 && simParams.mode5G
                   % BR of the next reserved resource (blind retransmission)
               ...
               if stationManagement.cv2xNumberOfReplicas(idVtx)>2 && hasFirstResourceThisTbeacon && BRnextnextReservedTransmission~=-1 && simParams.mode5G
                    % BR of the next reserved resource (blind retransmission)
                    stationManagement.knownUsedMatrixCV2X(BRnextnextReservedTransmission,idVrx) = 1;
                    % Power associated to the future reservation
                    % max between previous sensing and retransmission reservation
                    reservationRetransmissionPower = max(stationManagement.sensingMatrixCV2X(1,BRnextnextReservedTransmission,idVrx),stationManagement.sensingMatrixCV2X(1,BRtx,idVrx));
                    % Sensign matrix updated
                    stationManagement.sensingMatrixCV2X(1,BRnextnextReservedTransmission,idVrx) = reservationRetransmissionPower;
               end
...
%                % resources when reserving resources for retransmissions
               if phyParams.BRoverlapAllowed && stationManagement.cv2xNumberOfReplicas(idVtx)>1 && hasFirstResourceThisTbeacon && BRnextReservedTransmission~=-1 && simParams.mode5G
