...
if simParams.technology ~= constants.TECH_ONLY_11P % not only 11p
    if simParams.mode5G == constants.MODE_LTE
        ...
        phyParams.Tfr = 0.01;                        % LTE frame period (s)프레임 길이
        phyParams.Tsf = phyParams.Tfr/10;            % LTE subframe period (s)서브프레임 길이
        phyParams.Tslot = phyParams.Tsf/2;           % LTE time slot period (s)슬롯 길이
        phyParams.TsfGap = phyParams.Tsf * (1/14);   % LTE gap at the end of the subframe심볼의 길이
    ...
    % [cv2xCbrFactor]
    [phyParams,varargin] = addNewParam(phyParams,'cv2xCbrFactor',0.1,'Factor for CV2X DCC thresholds','double',fileCfg,varargin{1});
