...
if ismember(constants.V_STATE_LTE_TXRX, stationManagement.vehicleState(stationManagement.activeIDs))
   ...
   if ismember(simParams.BRAlgorithm, [constants.REASSIGN_BR_REUSE_DIS_SCHEDULED_VEH,...
    ...
    if simParams.BRAlgorithm == constants.REASSIGN_BR_STD_MODE_4
        ...
        TTI_BR = ceil(stationManagement.BRid(:,1)/appParams.NbeaconsF);
        stationManagement.BRid(:,1) = stationManagement.BRid(:,1) + (TTI_BR<=TTIGen) * appParams.Nbeacons;
%         stationManagement.BRid = sort(stationManagement.BRid,2);
        stationManagement.BRid(:,1) = stationManagement.BRid(:,1) - (stationManagement.BRid(:,1)>appParams.Nbeacons) * appParams.Nbeacons;
...
% DENM의 전송 횟수 조절 유무
stationManagement.DENM_l = zeros(length(stationManagement.activeIDs),1);

%% Initialization of buffer(Experience replay buffer of each vehicle)

expManagement.obs = zeros(simValues.maxID,200);
expManagement.action = cell(simValues.maxID,1);
expManagement.reward = zeros(simValues.maxID,1);
expManagement.previousObs = zeros(simValues.maxID,200);
expManagement.nextObs = zeros(simValues.maxID,200);
expManagement.isdone = zeros(simValues.maxID,1);
expManagement.exp = cell(simValues.maxID,5);
expManagement.savingExp = cell(simValues.maxID,5);

stationManagement.resource_col = zeros(simValues.maxID,1);
stationManagement.PDR_suc = zeros(simValues.maxID,1);
stationManagement.PDR_fai = zeros(simValues.maxID,1);
stationManagement.PDR = zeros(simValues.maxID,1);
stationManagement.PDR_num = 0;
stationManagement.sinrThreshold = 0;
stationManagement.MRC_count = zeros(simValues.maxID,simValues.maxID);
stationManagement.overtrans = zeros(simValues.maxID,1);
sinrManagement.sinr = zeros(simValues.maxID,1);